...
export default class Game extends Component {
  ...
  getGameImage = () => {
    ...
    let randomSearchItem =
      spaceSearch[Math.floor(Math.random() * spaceSearch.length)];
    let oneHundred = [];
    for (let i = 0; i <= 100; i++) {
      oneHundred.push(i);
    }
    let randomNumber =
      oneHundred[Math.floor(Math.random() * oneHundred.length)];
  ...
  playGame = () => {
    ...
    return spaceWords.map((word, index) => (
      <div className="guessing" key={index}>
        <button onClick={(e) => this.guessChoice(e)} id={word}>
          {word}
        </button>
      </div>
    ));
  ...
  guessChoice = (e) => {
    ...
    if (this.state.item === e.target.id) {
      ...
      $(".namegamebutton").html(
        "Wrong, Try Again. Correct Answer: " + this.state.item
      );
    }
  };
    ...
    return (
      <div className="namegame">
        <div className="titlegame">
          Guess which one is associated with this image:
        </div>
        <img src={this.state.image} id="namegameimage" alt="game" />
        {this.renderGame()}
